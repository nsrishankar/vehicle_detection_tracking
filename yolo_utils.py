...
def load_weights(model,weights):
    ind=0          
    raw_data=np.fromfile(weights,np.float32)
    raw_data=raw_data[4:]
    
    for layer in model.layers:
        shape=[w.shape for w in layer.get_weights()]
        if shape!=[]:
            k_shape,b_shape=shape
            bias=raw_data[ind:ind+np.prod(b_shape)].reshape(b_shape)
            ind+=np.prod(b_shape)
            kernel=raw_data[ind:ind+np.prod(k_shape)].reshape(k_shape)
            ind+=np.prod(k_shape)
